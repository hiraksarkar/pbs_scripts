...
read_root_dir="/gpfs/scratch/moamin/hirak_sra_contamination/reads/"
...
while read line ; do
	lstart=`date +%s`
 	##this line is not correct, should strip :port and store to ip var
  	...
  	fq1=${read_root_dir}/${line}/${line}_1.fastq.gz
  	fq2=${read_root_dir}/${line}/${line}_2.fastq.gz

	srrname=$( echo $(basename $fq1) | cut -d_ -f1)
	...
	runAgain=true
	if [ -s "${output}/${srrname}.1.fa" ] & [ -s "${output}/${srrname}.2.fa" ]; then
		runagain=false
...
###################### stop the aligner after n rows ######################
	if ps -p ${pid} > /dev/null; then
		n=1000000
		n=$((n*2))
		echo "${unmapped_read_printer} ${p1} ${n} ${pid} ${output}/${srrname}"
